...
namespace Neurotec.Samples
{
	public partial class IdentifyVoice : UserControl
	{
		...
		private void OnIdentifyDone(NBiometricStatus status)
		{
		...
		private async void BtnOpenTemplatesClickAsync(object sender, EventArgs e)
		{
			...
			if (openFileDialog.ShowDialog() == DialogResult.OK)
			{
				try
				{
					...
					await _biometricClient.ClearAsync(); // Clear previously enrolled subjects
		...
		private async void BtnOpenAudioFileClickAsync(object sender, EventArgs e)
		{
			btnIdentify.Enabled = false;
		...
		private async void BtnIdentifyClickAsync(object sender, EventArgs e)
		{
			...
			if (_subject != null && _subjects != null && _subjects.Length > 0)
			{
				try
				{
					var status = await _biometricClient.IdentifyAsync(_subject);
					OnIdentifyDone(status);
				}
				catch (Exception ex)
				{
					Utils.ShowException(ex);
