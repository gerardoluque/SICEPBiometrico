...
namespace Enrolar
{
    public partial class Form1 : Form
    {
        ...
        int m_captureDir = 0;
        int m_minCount = 0;
        public RSEType1Info info = new RSEType1Info();
        ...
        public void RegistraHuella(int dedo, byte[] imageData, byte[] templete, byte[] wsq)
        {
            #region RegistraHuella
            bool existeRegistro;
            string select = " SELECT 1 FROM BTS.HUELLA WHERE ESTADO = " + param[4] + " AND MUNICIPIO = " + param[5] + " AND CERESO = '" + param[6] + "' AND ANO = " + param[7] + " AND FOLIO = " + param[8] + " AND DEDO = " + dedo.ToString() + " AND NUM_INGRESO = " + param[9];
            string insert = " INSERT INTO BTS.HUELLA(ESTADO, MUNICIPIO, CERESO, ANO, FOLIO, DEDO, NUM_INGRESO, HUELLAIMAGEN, TEMPLATE, WSQ) VALUES (" + param[4] + "," + param[5] + ",'" + param[6] + "'," + param[7] + "," + param[8] + "," + dedo.ToString() + "," + param[9] + ", :BlobParameter, :BlobParameter2, :BlobParameter3 )";
            ...
            try
            {
                ...
                if (existeRegistro == false)
                {
                    ...
                    blobParameter3.OracleDbType = OracleDbType.Blob;
                    blobParameter3.ParameterName = "BlobParameter3";
                    blobParameter3.Value = wsq;
                    ...
                    cmdInsert.Parameters.Add(blobParameter3);
        ...
        private void clbDedo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (n == 4)
            {
                MessageBox.Show("No se pueden omitir todos los dedos.", "Aviso");
                clbDedo.SetItemChecked(clbDedo.SelectedIndex, false);
                return;
            }
        ...
        private void clbPulgar_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("No se pueden omitir todos los dedos.", "Aviso");
                clbPulgar.SetItemChecked(clbPulgar.SelectedIndex, false);
                return;
            }
